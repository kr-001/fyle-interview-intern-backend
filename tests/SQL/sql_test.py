...
def create_n_graded_assignments_for_teacher(self,number: int = 0, teacher_id: int = 1) -> int:
        """
...
class TestAssignments:
    ...
    def test_get_assignments_in_various_states(self):
        """Test to get assignments in various states"""
        # Print the state of the assignments table before any changes
        print(db.session.execute("SELECT * FROM assignments").fetchall())

        # Define the expected result before any changes
        expected_result = [('DRAFT', 4), ('GRADED', 1), ('SUBMITTED', 1)]
        ...
        # Print the state of the assignments table after the first query
        print(db.session.execute("SELECT * FROM assignments").fetchall())

        # Modify an assignment state and grade, then re-run the query and check the updated result
        expected_result = [('DRAFT', 4), ('GRADED', 2)]
        ...
        # Print the state of the assignments table after modifying an assignment
        print(db.session.execute("SELECT * FROM assignments").fetchall())
